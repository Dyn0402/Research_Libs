...
//	}
	...
	bin_lows.reserve(samples + 1);
	for (int i = 0; i < samples; i++) {
		bin_lows[i] = 2 * M_PI * r->Rndm();
	}
	sort(bin_lows.begin(), bin_lows.end());  // Sort bin edges such that algorithm can efficiency check each
	bin_lows[samples] = 6 * M_PI;  // Dummy value for index pulled at end of algorithm
...
//	}
	...
	vector<double> bin_lows(samples + 1);  // Stochastically generate sample number of bin edges
	for (int i = 0; i < samples; i++) {
		bin_lows[i] = 2 * M_PI * r->Rndm();
	}
	sort(bin_lows.begin(), bin_lows.end());  // Sort bin edges such that algorithm can efficiency check each
